...
const Home = () => {
      ...
      const getEventForDay = (day: Date) => events.find(e => isSameDay(new Date(e.event_date), day));
                        ...
                        {!founderDismissed && (
                            <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl animate-slide-up relative">
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                                      ...
                                                      <h2 className="text-xl font-bold text-foreground">A Message from Our Founders</h2>h2>
                                        </div>div>
                                        <div className="flex flex-col md:flex-row-reverse gap-6 items-start">
                                                      <div className="flex-1">
                                                                      <p className="text-foreground/90 leading-relaxed mb-4">
                                                                                        "Welcome to Her Story Collective! We created this space to celebrate the incredible women in our communities. Here, every story matters, every connection counts, and every woman has the power to inspire and be inspired. Together, we're building a network of support, growth, and empowerment."
                                                                      </p>p>
                                                                      <p className="text-foreground/80 text-sm italic">- Toya &amp; Jay, Co-founders</p>p>
                                                      </div>div>
                            ...
                            <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl animate-slide-up">
                                      <div className="flex items-center justify-between mb-4">
                                                  <h2 className="text-xl font-bold text-foreground">Upcoming Events</h2>h2>
                                    ...
                                    events.map(event => (
                                                            <div key={event.id} className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20 hover:bg-white/20 transition-all duration-300">
                                                                                  <h3 className="font-semibold text-foreground mb-1">{event.title}</h3>h3>
                                                                                  ...
                                                                                  <div className="flex items-center justify-between">
                                                                                                          <div className="flex items-center gap-2 text-foreground/70 text-sm">
                                                                                                                                    ...
                                                                                                                                    <span>{event.is_virtual ? 'Virtual' : (event.location || 'TBD')}</span>span>
                                                                                                              </div>div>
                                                                                                          <button
                                    ...
                                    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
                                                        <p className="text-foreground/80 text-sm mb-3">No upcoming events yet.</p>p>
                                                        <button
                                                                                  onClick={() => navigate('/events')}
                                      ...
                                      <div key={d} className="text-foreground/70 font-semibold p-2">{d}</div>div>
                                    ))}
                                    ...
                                    <div className="mt-3 space-y-1">
                                        {events.map(e => (
                                                              <div key={e.id} className="flex items-center gap-2 text-xs text-foreground/70">
                                                                                      ...
                                                                                      <span>{e.title} ({format(new Date(e.event_date), 'MMM d')})</span>span>
                                                              </div>div>
                                                            ))}
                                    </div>div>
                                                                                )}
                            ...
                            <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl animate-slide-up">
                                      <div className="flex items-center justify-between mb-4">
                                                  <h2 className="text-xl font-bold text-foreground">Spotlight Members</h2>h2>
                                                  <Star className="w-6 h-6 text-foreground/70" />
                                      </div>div>
                                      <div className="space-y-4">
                                          {spotlightMembers.length > 0 ? (
                                spotlightMembers.map((member: any) => (
                                                    <div key={member.user_id} className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20 hover:bg-white/20 transition-all duration-300">
                                                                      <div className="flex items-center space-x-4">
                                                                                          ...
                                                                                          <div className="flex-1">
                                                                                                                <h3 className="font-semibold text-foreground">{member.first_name_initial}</h3>h3>
                                                                                              {member.city_only && (
                                                                                <div className="flex items-center gap-1 text-foreground/70 text-xs">
                                                                                                          <MapPin className="w-3 h-3" />
                                                                                                          <span>{member.city_only}</span>span>
                                                                                    </div>div>
                                                                                                                )}
                                                                                              {member.interests_sample && member.interests_sample.length > 0 && (
                                                                                <div className="flex flex-wrap gap-1 mt-1">
                                                                                    {member.interests_sample.slice(0, 2).map((interest: string, idx: number) => (
                                ...
                                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 text-center">
                                                <p className="text-foreground/80 text-sm mb-3">No members to spotlight yet.</p>p>
                                                <p className="text-foreground/60 text-xs">Complete your profile to appear here and connect with others!</p>p>
                                </div>div>
                                                  )}
                                      </div>div>
                                      <button
